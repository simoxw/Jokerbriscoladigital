...
const ScoreBoard: React.FC<ScoreBoardProps> = ({ players, type }) => {
      ...
      <div className="flex justify-around items-center text-[10px]">
        {players.map(p => (
          <div key={p.id} className="flex flex-col items-center leading-none">
            <span className="text-slate-400 scale-90 origin-bottom mb-0.5">{p.name === 'Tu' ? 'TU' : `I${p.index}`}</span>
            <span className={`font-mono font-bold text-xs ${p.name === 'Tu' ? 'text-amber-400' : 'text-white'}`}>
